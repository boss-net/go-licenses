...
func doCheckCmd(_ *cobra.Command, args []string) error {
	var rules chklicenses.Rules
	var err error
	switch {
	case len(appConfig.Permit) > 0:
		rules, err = chklicenses.NewRules(chklicenses.AllowAction, appConfig.Permit, appConfig.IgnorePkg...)
		fmt.Printf("Allow Rules: %+v\n", appConfig.Permit)
	case len(appConfig.Forbid) > 0:
		rules, err = chklicenses.NewRules(chklicenses.DenyAction, appConfig.Forbid, appConfig.IgnorePkg...)
	...
	licenseFinder := chklicenses.NewLicenseFinder(paths, gitRemotes, 0.9)
